...
def get_public_stats():
    conn = get_conn()
    cur = conn.cursor()
    stats = {"users": 0, "orders": 0, "approved_orders": 0}
    try:
        ...
        stats["approved_orders"] = cur.fetchone()["n"]
    except Exception:
        pass
    conn.close()
    return stats
...
def index():
    user = current_user()
    history = load_history_records(limit=7)
    latest_summary = summarize_snapshot(history[0]) if history else None
    last3 = [summarize_snapshot(h) for h in history[:3]]
    feedback_items = get_public_feedback(limit=3)
    public_stats = get_public_stats()
    return render_template("index.html", user=user, plans=PLANS, latest_summary=latest_summary, last3=last3, feedback_items=feedback_items, public_stats=public_stats)
...
def health():
    try:
        ...
        return {"status": "ok", "version": "v7.3-conversion-final", "users": n, "orders": o, "signals": len(signals), "source": source, "source_label": source_label(source), "generated_at": generated_at, "history_files": history_files}
    except Exception as e:
